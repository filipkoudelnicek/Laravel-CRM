  ...
  <div class="col-lg-10">
    <div class="card mb-4">
      <div class="card-header pb-0">
        <h5 class="mb-0">Upravit fakturu {{ $invoice->invoice_number }}</h5>
      </div>
      <div class="card-body">
        ...
        <form method="POST" action="{{ route('invoices.update', $invoice) }}" id="invoiceForm">
            ...
            <div class="col-md-3 mb-3">
              ...
              <select name="status" class="form-select" required>
                ...
                @foreach(\App\Models\Invoice::STATUSES as $s)
                  <option value="{{ $s }}" {{ old('status', $invoice->status) === $s ? 'selected' : '' }}>{{ $invStatusLabels[$s] ?? ucfirst($s) }}</option>
                @endforeach
              </select>
            </div>
            <div class="col-md-3 mb-3">
              <label class="form-label">Měna</label>
              <input type="text" name="currency" value="{{ old('currency', $invoice->currency) }}" class="form-control" maxlength="3" required>
            </div>
            <div class="col-md-3 mb-3">
              <label class="form-label">DPH %</label>
              <input type="number" name="tax_rate" value="{{ old('tax_rate', $invoice->tax_rate) }}" step="0.01" min="0" max="100" class="form-control" required>
            </div>
            <div class="col-md-3 mb-3">
              <label class="form-label">Datum vystavení</label>
              <input type="date" name="issued_at" value="{{ old('issued_at', $invoice->issued_at?->format('Y-m-d')) }}" class="form-control">
            </div>
            <div class="col-md-3 mb-3">
              <label class="form-label">Splatnost</label>
              <input type="date" name="due_at" value="{{ old('due_at', $invoice->due_at?->format('Y-m-d')) }}" class="form-control">
            </div>
            <div class="col-md-3 mb-3">
              <label class="form-label">Datum úhrady</label>
              <input type="date" name="paid_at" value="{{ old('paid_at', $invoice->paid_at?->format('Y-m-d')) }}" class="form-control">
            </div>
          </div>
          ...
          <div class="d-flex justify-content-end gap-2 mt-4">
            <a href="{{ route('invoices.show', $invoice) }}" class="btn btn-outline-secondary btn-sm">Zrušit</a>
