...
@section('content')
<x-form-layout 
  title="Upravit fakturu {{ $invoice->invoice_number }}" 
  ...
  <form method="POST" action="{{ route('invoices.update', $invoice) }}" id="invoiceForm">
      ...
      <div class="col-md-3">
        <x-form-field name="status" label="Stav" type="select" required>
          ...
          @foreach(\App\Models\Invoice::STATUSES as $s)
            <option value="{{ $s }}" @selected(old('status', $invoice->status) === $s)>{{ $invStatusLabels[$s] ?? ucfirst($s) }}</option>
          @endforeach
        </x-form-field>
      </div>
      <div class="col-md-3">
        <x-form-field name="currency" label="Měna" value="{{ old('currency', $invoice->currency) }}" maxlength="3" required />
      </div>
      <div class="col-md-3">
        <x-form-field name="tax_rate" label="DPH %" type="number" step="0.01" min="0" max="100" value="{{ old('tax_rate', $invoice->tax_rate) }}" required />
      </div>
      <div class="col-md-3">
        <x-form-field name="issued_at" label="Datum vystavení" type="date" value="{{ old('issued_at', $invoice->issued_at?->format('Y-m-d')) }}" />
      </div>
    </div>

    <div class="row">
      <div class="col-md-3">
        <x-form-field name="due_at" label="Splatnost" type="date" value="{{ old('due_at', $invoice->due_at?->format('Y-m-d')) }}" />
      </div>
      <div class="col-md-3">
        <x-form-field name="paid_at" label="Datum úhrady" type="date" value="{{ old('paid_at', $invoice->paid_at?->format('Y-m-d')) }}" />
      </div>
    </div>

    <h6 class="mt-4 mb-3">Položky faktury</h6>
    <div id="items-container">
      @php $oldItems = old('items', $invoice->items->map(fn($it) => $it->toArray())->toArray()); @endphp
    ...
    <div class="row mt-4">
      <div class="col-md-8">
        <x-form-field name="notes" label="Poznámky" type="textarea" rows="3" value="{{ old('notes', $invoice->notes) }}" />
      </div>
      <div class="col-md-4">
    ...
    <x-form-actions submitText="Uložit změny" backUrl="{{ route('invoices.show', $invoice) }}" />
  </form>
</x-form-layout>
